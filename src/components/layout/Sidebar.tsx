    ...
    items: [
      {
        icon: "/home.png",
        label: "Dashboard",
        href: "", // resolved dynamically per role
...
export default function Sidebar() {
  ...
  if (!user) {
    return (
      <aside className="w-16 lg:w-64 bg-white border-r p-4">
        <p className="text-gray-400 text-sm hidden lg:block">Please log in</p>
      </aside>
    );
  }
  ...
  return (
    <aside className="w-16 lg:w-64 bg-white border-r flex flex-col h-full overflow-y-auto">
      ...
      <div className="hidden lg:flex flex-col gap-1 px-4 py-3 border-b bg-gray-50">
        ...
        <span
          className={`text-xs font-medium px-2 py-0.5 rounded-full w-fit ${roleBadgeColors[role]}`}
        >
          {roleLabels[role]}
        </span>
      </div>
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.title} className="mb-4">
              <p className="hidden lg:block text-xs font-semibold text-gray-400 uppercase tracking-wider px-2 mb-1">
                {group.title}
              </p>
